...
async function writeTests(newTestsList) {
  ...
  for (const t of deleted) {
    if (!t.zephyrId || t.zephyrId === '') continue;
    try {
      await axios.delete(`https://api.zephyrscale.smartbear.com/v2/testcases/${t.zephyrId}`, { headers });
    } catch (e) {
      console.error(`Failed to delete Zephyr test case ${t.zephyrId}:`, e.response?.data || e.message);
    }
  }
