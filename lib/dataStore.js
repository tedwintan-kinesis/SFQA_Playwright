...
async function readData(filename) {
  ...
  if (IS_PROD) {
    ...
    try {
      const { data } = await octokit.repos.getContent({ owner, repo, path: `data/${filename}` });
      const raw = Buffer.from(data.content, 'base64').toString('utf-8');
      return JSON.parse(raw);
    } catch (e) {
      console.error(`[DataStore] GitHub read error for ${filename}:`, e.message);
      throw e;
    }
  } else {
...
async function readTests() {
  try {
    return await readData('tests.json');
  } catch (e) {
    return [];
  }
}
...
async function readVariables() {
  try {
    return await readData('variables.json');
  } catch (e) {
    return [];
  }
}
...
async function readTestVariables() {
  try {
    return await readData('test_variables.json');
  } catch (e) {
    return [];
  }
}
...
async function readRuns() {
  try {
    return await readData('runs.json');
  } catch (e) {
    return [];
  }
}
...
async function readSuites() {
  try {
    return await readData('suites.json');
  } catch {
    return [];
  }
}
...
async function readConfig() {
  try {
    return await readData('config.json');
  } catch {
...
async function generateSpecContent(name, url, zephyrId, steps = [], variables = null) {
...
async function updateSpecContent(content, name, url, zephyrId, steps = [], variables = null) {
  if (steps && steps.length > 0) {
    return await generateSpecContent(name, url, zephyrId, steps, variables);
  }
