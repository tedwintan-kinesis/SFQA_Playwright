...
async function writeRuns(newRunsList, forceZephyr = false) {
  let currentRuns = [];
  try {
    currentRuns = await readData('runs.json');
  } catch (e) {}

  const added = forceZephyr ? newRunsList : newRunsList.filter(r => !currentRuns.some(cr => cr.id === r.id));

  // 1. Always save to local/github dashboard database
  if (IS_PROD) {
    await writeDataGitHub('runs.json', newRunsList);
  } else {
    writeDataLocal('runs.json', newRunsList);
  }

  // 2. If Zephyr is configured, push the new runs
  const token = process.env.ZEPHYR_TOKEN;
  if (!token) return;
  ...
  for (const r of added) {
    try {
      if (!r.zephyrId || r.zephyrId === '-') {
        console.log(`[Zephyr] Skipping run ${r.id} — no zephyrId`);
        continue;
      }

      let cycleKey = r.zephyrCycle;
      if (!cycleKey || cycleKey === '-') {
        console.log(`[Zephyr] No cycle on run ${r.id}, fetching latest cycle...`);
        ...
        if (cycles[0]) {
          cycleKey = cycles[0].key;
          console.log(`[Zephyr] Using latest cycle: ${cycleKey}`);
        }
      }

      if (!cycleKey || cycleKey === '-') {
        console.log(`[Zephyr] No cycle found for run ${r.id}, skipping Zephyr push`);
        continue;
      }
      ...
      const match = currentRuns.find(cr => cr.id === r.id) || (await readTests()).find(t => t.id === r.testId || t.zephyrId === r.zephyrId);
      if (match && match.steps) {
        testScriptResults = match.steps.map((step, idx) => ({
          statusName: r.status === 'passed' ? 'Pass' : (r.status === 'running' || r.status === 'incomplete') ? 'In Progress' : 'Fail',
          actualResult: `Step ${idx + 1} (${step.action}) ${r.status.toUpperCase()}.`
        }));
      }

      // Parse duration safely — handles "17s", "17", or number
      ...
      if (testScriptResults.length > 0) {
        payload.testScriptResults = testScriptResults;
      }

      console.log(`[Zephyr] Pushing execution for ${r.zephyrId} to cycle ${cycleKey}, status=${payload.statusName}`);
      const zRes = await axios.post('https://api.zephyrscale.smartbear.com/v2/testexecutions', payload, { headers });
      let execKey = zRes.data?.key;
      let execId = zRes.data?.id;

      if (!execKey && execId) {
        try {
          const fetchRes = await axios.get(`https://api.zephyrscale.smartbear.com/v2/testexecutions/${execId}`, { headers });
          if (fetchRes.data?.key) execKey = fetchRes.data.key;
        } catch (err) {
          console.error(`[Zephyr] Failed to fetch execution details for ID ${execId}`);
        }
      }
      
      execKey = execKey || execId;
      console.log(`[Zephyr] Push success — execution key: ${execKey}`);
      
      if (execKey) {
        const runRef = newRunsList.find(n => n.id === r.id);
        if (runRef) {
      ...
      console.error(`[Zephyr] Failed to push run ${r.id}:`, e.response?.data || e.message);
    }
  }

  // If we enriched the runs with Zephyr IDs, save again
  if (didUpdateArray) {
    if (IS_PROD) await writeDataGitHub('runs.json', newRunsList);
    else writeDataLocal('runs.json', newRunsList);
  }
}
...
async function updateRun(runId, updates, skipZephyr = false) {
  ...
  const updatedRun = { ...runs[runIndex], ...updates, completedAt: new Date().toISOString() };
  runs[runIndex] = updatedRun;
  
  if (IS_PROD) await writeDataGitHub('runs.json', runs);
  else writeDataLocal('runs.json', runs);

  // Push update to Zephyr manually with force flag unless skipped
  if (!skipZephyr) {
    await writeRuns([updatedRun], true); 
  } 

  // Also update test status if it is passed/failed/incomplete
  if (['passed', 'failed', 'incomplete'].includes(updatedRun.status)) {
